/*
  # Add Supervisor Notifications System

  1. Changes
    - Add notifications table for storing supervisor notifications
    - Add trigger for creating notifications on new requests
    - Add function to handle notification creation

  2. Notes
    - Notifications are created for both time and planner requests
    - Each notification is linked to specific supervisors and requests
    - Includes request type, employee info, and status
*/
...
CREATE POLICY "Supervisors can view their notifications"
  ON supervisor_notifications
  FOR SELECT
  TO authenticated
  USING (supervisor_id = auth.uid());

CREATE POLICY "Supervisors can update their notifications"
  ON supervisor_notifications
  FOR UPDATE
  TO authenticated
  USING (supervisor_id = auth.uid())
